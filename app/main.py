...
def run():
    ...
    hours_ahead = int(os.getenv("HOURS_AHEAD", "240"))  # default 10 days
    book_pref = os.getenv("BOOK_FILTER", "bet365")

    sc = SheetClient(sheet_name)

    # 1) Fixtures (next N hours using next=50 per league)
    ...
    # 3) Write to sheets (keep model/picks empty for now)
    try:
        if fixtures.empty:
